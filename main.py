...
def draw_arc(v, circle_center, circle_radius, down_vector, max_degree=30, precision=0.7):
    ob = v - circle_center
    crossValue = ti.math.dot(ob, down_vector)
    degree = ti.math.degrees(ti.acos(crossValue / (down_vector.norm() * ob.norm())))
    return ti.abs(ti.math.distance(v, circle_center) - circle_radius) <= precision and degree < max_degree
...
def initialize_voxels():
    ...
    line_height_low = big_ring_center[1] + big_radius
    ...
    for i, j in ti.ndrange(n, n):
        ...
        if draw_radius(v, big_ring_center, big_radius, 0.7) \
                or draw_radius(v, ring1_center, small_ring_radius) \
                or draw_radius(v, ring2_center, small_ring_radius) \
                or draw_radius(v, ring3_center, small_ring_radius) \
                or draw_arc(v, arc1_center, arc_center1_radius, down_vector, 45, 0.5) \
                or draw_arc(v, arc2_center, arc_center2_radius, down_vector, 60, 0.5) \
                or draw_arc(v, arc_big_center, arc_center3_radius, down_vector, 30) \
                or draw_line(v, line_height_top, line_height_low, 2):
            random = ti.random()
            if random < 0.001:
